...
class _ListViewPrePedidosState extends State<ListViewPrePedidos> {
  ...
  Widget build(BuildContext context) {
       ...
       floatingActionButton: FloatingActionButton(
        onPressed:() {
           Navigator.push(context, MaterialPageRoute(
                          builder: (context) => ConfirmarDireccionYPedido(userDoc: widget.docUsu,
                                                 )
                        ));
        },
   ...
   StreamBuilder<QuerySnapshot> _recuperarPrePedidos() {
    ...
    return new StreamBuilder<QuerySnapshot>(
      stream: Firestore.instance.collection('usuarios').document(widget.docUsu.documentID).collection('prePedidosUsu').snapshots(),      
                                        ...
                                        showDialog(
                                          ...
                                          builder: (BuildContext context){
                                            return AlertDialog(
                                              ...
                                              content: new Text("¿Realmente desea eliminar el prodcuto ${prePedDoc.data['nombre_pro']}?"),
                                              actions: <Widget>[
                                                ...
                                                FlatButton(
                                                  child: Text("ACEPTAR"),
                                                  onPressed: (){
                                                        Firestore.instance.collection('usuarios').document(widget.docUsu.documentID).collection('prePedidosUsu').document(prePedDoc.documentID).delete();        
                                                        Navigator.of(context).pop(); 
                                                  },
                                                )
                                              ],
  ...
  Future<Null> _verProductoDialog(BuildContext context, DocumentSnapshot prePedDoc) {
          ...
          content: Container(
            ...
            child: ListView(
              children: <Widget>[
                Divider(height: 20,),
                Text("Descripción:"),
                Text("${prePedDoc['descripcion_pro']}", style: TextStyle(fontSize: 20.0),),
                Divider(height: 20,),
                Text("Precio:"),
                Text("\$${prePedDoc['precio_pro']}", style: TextStyle(fontSize: 20.0)),
                Divider(height: 20,),
                SizedBox(height: 10,),
                Text("Imagen:"),
                SizedBox(height: 15,),
                FlatButton(
                  child: Image.network("${prePedDoc.data["imagen_pro"]}", width: 250.0,),
                  onPressed: (){},
                )
